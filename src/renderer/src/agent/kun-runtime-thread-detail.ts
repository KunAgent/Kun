...
export async function getKunThreadDetail(threadId: string, options: {
  ...
  const turns = (Array.isArray(thread.turns) ? thread.turns : [])
    .filter((turn) => sourceHistoryAllowed() || !turn.id.startsWith('codex:'))
  ...
  const nativeTurns = turns.filter((turn) => !turn.id.startsWith('codex:'))
  ...
  const resolvedLatestUserMessageId =
    latestUserMessageId ?? [...items].reverse().find((item) =>
      item.kind === 'user_message' && !item.turnId?.startsWith('codex:'))?.id
  return {
