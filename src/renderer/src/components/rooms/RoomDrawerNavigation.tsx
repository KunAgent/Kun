...
export function useRoomDrawerNavigation(roomId: string | null) {
  ...
  return { frames, open, back, close, section: (section: RoomDetailsSection) => open({ kind: 'section', section }, true) }
}
...
export function RoomDrawerNavigation({ frames, onBack, onClose, onSection, render }: {
  ...
  const title = current.target.kind === 'reply' ? t('roomsReplyThreadTitle') : current.target.kind === 'content' ? t('roomsReplyContentTitle') : undefined
  return <RoomDetailsDrawer section={section?.kind === 'section' ? section.section : 'discussion'}
    ...
    childOpen={current.target.kind === 'reply' || current.target.kind === 'content' || frames.length > 1}
