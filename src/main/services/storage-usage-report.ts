...
async function scanCategory(
  ...
  // Do not follow a caller-provided symlink/junction at the root either.
  // Child entries are inspected with lstat below, but opendir(root) would
  // otherwise resolve the link before we have a chance to reject it.
  try {
    const rootMetadata = await lstat(root)
    if (rootMetadata.isSymbolicLink()) return result
  } catch (error) {
    if (!isMissing(error)) result.error = describeError(error)
    return result
  }
