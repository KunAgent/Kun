...
import { SanottsWorkerHost, SANOTTS_WORKER_START_TIMEOUT_MS, SANOTTS_WORKER_RUN_TIMEOUT_MS } from './local-sanotts-worker-host'
...
function setup() {
  ...
  const host = new SanottsWorkerHost(() => {
...
const reply = (worker: FakeWorker, id: string) => worker.emit('message', {
  type: 'result', id, ok: true,
  pcm: new Int16Array(1), sampleCount: 1, sampleRate: 22_050, durationSeconds: 1 / 22_050
})
...
it('bounds startup time and allows a later retry', async () => {
  ...
  await flush()
  await vi.advanceTimersByTimeAsync(SANOTTS_WORKER_START_TIMEOUT_MS)
...
it('bounds native execution time and resolves the pending run', async () => {
  ...
  workers[0].emit('message', { type: 'ready' })
  await flush()
  await vi.advanceTimersByTimeAsync(SANOTTS_WORKER_RUN_TIMEOUT_MS)
  expect(await result).toMatchObject({ ok: false, message: 'sanoTTS synthesis timed out' })
