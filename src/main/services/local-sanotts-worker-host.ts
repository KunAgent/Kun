...
import type { SanottsWorkerRequest, SanottsWorkerResponse } from './local-sanotts-worker-protocol'

export const SANOTTS_WORKER_START_TIMEOUT_MS = 30_000
export const SANOTTS_WORKER_RUN_TIMEOUT_MS = 120_000

type Result = Extract<SanottsWorkerResponse, { type: 'result' }>
type Run = Extract<SanottsWorkerRequest, { type: 'synthesize' }>
...
export class SanottsWorkerHost {
  ...
  private ensure(): Generation {
    ...
    const timer = setTimeout(() => this.fail(generation, 'sanoTTS worker startup timed out'), SANOTTS_WORKER_START_TIMEOUT_MS)
    void ready.then(() => clearTimeout(timer), () => clearTimeout(timer))
    worker.on('message', (message: SanottsWorkerResponse) => {
    ...
    worker.on('exit', () => this.fail(generation, 'sanoTTS speech worker stopped'))
    return generation
  }
  ...
  run(request: Run, canceled: () => boolean): Promise<Result> {
    ...
    const task = this.queue.then(async (): Promise<Result> => {
      ...
      return new Promise<Result>((resolve) => {
        const timer = setTimeout(() => this.fail(generation, 'sanoTTS synthesis timed out'), SANOTTS_WORKER_RUN_TIMEOUT_MS)
  ...
  cancel(requestId: string): void {
    ...
    // WASM inference cannot process a cancel message while blocking its thread.
    this.fail(generation, 'sanoTTS speech canceled')
  }
  ...
  async reset(): Promise<void> {
    ...
    this.fail(generation, 'sanoTTS speech worker reset')
