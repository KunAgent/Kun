...
describe('scanStorageUsage', () => {
  ...
  it('does not follow a symlink used as the category root', async () => {
    const root = await tempRoot()
    const outside = await tempRoot()
    await writeFile(join(outside, 'secret.txt'), 'secret')
    const linkedRoot = join(root, 'linked-root')
    const created = await symlink(outside, linkedRoot).then(() => true).catch(() => false)
    if (!created) return

    const report = await scanStorageUsage({ roots: { attachments: linkedRoot } })
    const attachments = report.entries.find((entry) => entry.category === 'attachments')!
    expect(attachments).toMatchObject({ bytes: 0, files: 0, directories: 0, truncated: false })
    expect(attachments).not.toHaveProperty('error')
  })
