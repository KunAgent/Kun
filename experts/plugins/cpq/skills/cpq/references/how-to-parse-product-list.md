...
- **首行（必填，门控）**：`<!-- site: cn version=2 -->` 或 `<!-- site: intl version=2 -->`，取值来自主流程"启动判断"中已锁定的 `site`
  - `version=2` 是 Phase 1 重构后版本号；老版本（无 version 字段）会被 [`check-phase1.mjs`](../scripts/check-phase1.mjs) 拒绝（决策 8）
...
| #   | row_id | status | 产品名（从输入提取） | 规格/子类型（展开后单项） | 搜索关键词（产品原文+子项） | 站点 | 地域 | 来源判断 | 售卖模式 | 优惠策略 | 返佣（%） | 约束条件 | 推断标记 |
| --- | ------ | ------ | -------------------- | ------------------------- | --------------------------- | ---- | ---- | -------- | -------- | -------- | --------- | -------- | -------- |
...
3. **与后续展开的顺序**：合并单元格展平是**最先执行**的结构性动作（B.1），先把合并组摊平成字段齐备的行，再依次做 B.2 伴生拆分 → B.3 规格展开 → B.4 地域笛卡尔积。
...
### 阶段 B.2 · 伴生产品拆分（强制 · 决策 4）

主算法直接读 [`data/phase1-token-dict/companion-trigger.md`](./data/phase1-token-dict/companion-trigger.md) 字典做触发判断；详细拆分规则、易混淆场景、反模式见 [`how-to-identify-companion-products.md`](./how-to-identify-companion-products.md)。

**执行步骤摘要**（详见独立文档）：

```
对每个主产品行 R:
  1. 加载 companion-trigger.md（按 site 过滤）
  2. 扫描 R.规格/子类型 + R.约束条件 中的所有 token
  3. 命中【强制清单】触发词 → 拆出独立行 R'
  4. 命中【可选清单】触发词 → 拆出独立行 + companion_inferred=yes
  5. 主产品行 R 移除已拆出的 token（搜索关键词重新构造）
  6. 累计 companion_expanded 计数
```

完成 B.2 后填入 phase1-done：`companion_expanded=<C>`、`step_companion_expanded=<yes|n/a>`。
...
- 伴生拆分结果（`companion_expanded` 额外行数，若 > 0）
- 总条目数 / 腾讯云条目数 / 友商条目数
...
- ❌ 禁止下游 Phase 重新执行 Phase 1（重解析、重追问、重伴生拆分等）
