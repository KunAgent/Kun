...
`@kun/extension-api` 当前版本为 `1.5.0`；React 和 test SDK 包保持 `1.4.0`，均对应 Extension API major 1。Host 继续接受 v1.4 至 v1.0 Manifest。v1.5 新增全局 Agent 容量快照，以及受独立权限控制的只读 Rooms API；既有 Manifest 无需迁移。Manifest 字段以生成的 [JSON Schema](../../packages/extension-api/schema/kun-extension.schema.json) 和 `ExtensionManifestSchema` 为机器真源；Host API、事件和 payload 以发布包的 `.d.ts` 与 runtime Schema 为真源。
...
| `secrets` | 需 `storage.secrets` 的 Node Host 受保护字符串存储；View 不可直接访问 |
| `ui` | theme、locale、View state、Host message、通知和主会话上下文挂载 |
| `agent`, `threads` | 需 `agent.capacity.read` 的全局容量；扩展自有 Agent run、事件、steer/cancel 和 thread projection |
| `rooms` | 需 `rooms.read` 的房间、消息、任务与事件只读投影 |
...
| --- | --- | --- | --- | --- |
| `@kun/extension-api` | `1.5.0` | `.`<br>`./manifest.schema.json` | 567 | `61cd434273bbcc962dfa974e19120a7493247b2f79352186edd648947b9dfdb6` |
...
| `@kun/extension-api` | `agent` | `AgentBudgetSchema`<br>`AgentCancelRequestSchema`<br>`AgentCapacitySnapshotSchema`<br>`AgentCreateRunRequestSchema`<br>`AgentCreateRunResponseSchema`<br>`AgentInputSchema`<br>`AgentListRunEventsRequestSchema`<br>`AgentListRunEventsResponseSchema`<br>`AgentModelOptionSchema`<br>`AgentMutationResultSchema`<br>`AgentProfileDeclarationSchema`<br>`AgentReasoningEffortSchema`<br>`AgentRunEventSchema`<br>`AgentRunOptionsSchema`<br>`AgentRunSchema`<br>`AgentRunStateSchema`<br>`AgentSteerRequestSchema`<br>`AgentSubscribeRequestSchema`<br>`ExtensionThreadProjectionSchema`<br>`ExtensionVisibilitySchema`<br>`ListOwnThreadsRequestSchema`<br>`ListOwnThreadsResponseSchema`<br>`ResolvedAgentProfileSchema` | `AgentBudget`<br>`AgentCancelRequest`<br>`AgentCapacitySnapshot`<br>`AgentCreateRunRequest`<br>`AgentCreateRunResponse`<br>`AgentInput`<br>`AgentListRunEventsRequest`<br>`AgentListRunEventsResponse`<br>`AgentModelOption`<br>`AgentMutationResult`<br>`AgentProfileDeclaration`<br>`AgentProfileDeclarationInput`<br>`AgentReasoningEffort`<br>`AgentRun`<br>`AgentRunEvent`<br>`AgentRunOptions`<br>`AgentRunState`<br>`AgentSteerRequest`<br>`AgentSubscribeRequest`<br>`ExtensionThreadProjection`<br>`ExtensionVisibility`<br>`ListOwnThreadsRequest`<br>`ListOwnThreadsResponse`<br>`ResolvedAgentProfile` |
...
| `@kun/extension-api` | `rooms` | `RoomEventSchema`<br>`RoomEventsListRequestSchema`<br>`RoomEventsListResponseSchema`<br>`RoomListRequestSchema`<br>`RoomListResponseSchema`<br>`RoomMessageAttachmentSchema`<br>`RoomMessageSchema`<br>`RoomMessagesListRequestSchema`<br>`RoomMessagesListResponseSchema`<br>`RoomSummarySchema`<br>`RoomTaskCountsSchema`<br>`RoomTasksListRequestSchema`<br>`RoomTasksListResponseSchema`<br>`RoomTaskStatusSchema`<br>`RoomTaskSummarySchema` | `RoomEvent`<br>`RoomEventsListRequest`<br>`RoomEventsListResponse`<br>`RoomListRequest`<br>`RoomListResponse`<br>`RoomMessage`<br>`RoomMessageAttachment`<br>`RoomMessagesListRequest`<br>`RoomMessagesListResponse`<br>`RoomsApi`<br>`RoomSummary`<br>`RoomTaskCounts`<br>`RoomTasksListRequest`<br>`RoomTasksListResponse`<br>`RoomTaskStatus`<br>`RoomTaskSummary` |
