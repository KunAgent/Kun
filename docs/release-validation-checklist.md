...
- PR 和正式发布不再自动执行从 0.3.7 启动 GUI、升级、重启并观察窗口的整套验收；`smoke-gui-upgrade.cjs` 保留为手工诊断工具。
- 类型检查、lint、单元测试、依赖审计、各平台打包及签名公证继续保留。
- 发布前确认候选标签对应的提交及草稿状态，逐个下载 Windows、macOS 双架构、Linux 双架构的全部安装包，核对大小、清单 SHA-512 和 GitHub 资产 SHA-256。
- 更新 R2 后再次下载并核验 stable 和兼容 latest 通道，成功后才公开 GitHub Release。校验记录标记为 `artifact-integrity`，不表示已经通过 GUI 升级验证。
- 同版本草稿重建可能命中旧 CDN 缓存。重新发布候选时应清理旧缓存，或为下载链接加入绑定新构建的唯一查询参数；不得忽略哈希不匹配。
