...
describe('built-in subagent surfaces', () => {
  it('publishes exactly 47 complete built-in profiles', () => {
    expect(BUILTIN_AGENT_CATALOG).toHaveLength(47)
    expect(Object.keys(BUILTIN_SUBAGENT_PROFILES)).toHaveLength(47)
    ...
    expect(BUILTIN_AGENT_CATALOG.filter((entry) => entry.family === 'write')).toHaveLength(7)
