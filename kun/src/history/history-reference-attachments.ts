import { openCodeAttachmentDescriptors } from './opencode-projection.js'
...
export async function readHistoryAttachment(
  ...
  const attachment = (reference.provider === 'opencode' ? openCodeAttachmentDescriptors : reference.provider === 'claude-code' ? claudeAttachmentDescriptors : codexAttachmentDescriptors)(source.record).find((item) => item.index === attachmentIndex)
