...
export type ItemTailRepairResult = {
  repaired: boolean
  truncatedBytes: number
  appendedNewline: boolean
}

export async function ensureItemTailReady(input: {
  verified: Set<string>
  threadId: string
  path: string
  evidencePath: string
  ...
  await repairIncompleteItemTail(input)
  input.verified.add(input.threadId)
}

/**
 * Complete a valid final item record that lost only its JSONL newline. Other
 ...
 */
export async function repairIncompleteItemTail(input: {
  path: string
  evidencePath: string
}): Promise<ItemTailRepairResult> {
  let handle
  try {
  ...
  try {
    ...
    try {
      const parsed = TurnItem.safeParse(JSON.parse(tail.toString('utf8')))
      if (parsed.success) {
