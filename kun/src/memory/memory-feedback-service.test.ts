...
describe('MemoryFeedbackService correction', () => {
  ...
  it('abandons a prepared receipt when the previous memory was purged', async () => {
    const harness = await createHarness()
    await harness.memories.createWithId('mem_purged', {
      content: 'Purged before correction applied', scope: 'workspace', workspace: 'workspace-a'
    })
    const interrupted = new MemoryFeedbackService({
      dataDir: harness.root,
      memoryStore: harness.memories,
      feedbackStore: harness.feedback,
      config: feedbackConfig,
      nowIso: () => NOW,
      afterReceiptPrepared: async () => { throw new Error('interrupt after receipt') }
    })
    const request = correctionRequest('correct-purged', 'mem_purged')
    await expect(interrupted.correct(request)).rejects.toThrow('interrupt after receipt')

    await harness.memories.purge('mem_purged')

    await expect(harness.service.ready()).resolves.toBeUndefined()
    await expect(harness.service.ready()).resolves.toBeUndefined()
    const receiptDir = join(harness.root, 'memory-feedback-corrections')
    const [receiptName] = await readdir(receiptDir)
    const receipt = JSON.parse(await readFile(join(receiptDir, receiptName!), 'utf8')) as { state: string }
    expect(receipt.state).toBe('abandoned')
    expect(receipt).not.toHaveProperty('request')
    await expect(harness.service.correct(request)).rejects.toMatchObject({ code: 'inactive' })
  })

  it('keeps a prepared receipt retryable while the previous memory is disabled', async () => {
    const harness = await createHarness()
    await harness.memories.createWithId('mem_transient', {
      content: 'Disabled before correction applied', scope: 'workspace', workspace: 'workspace-a'
    })
    const interrupted = new MemoryFeedbackService({
      dataDir: harness.root,
      memoryStore: harness.memories,
    ...
    const request = correctionRequest('correct-transient', 'mem_transient')
    await expect(interrupted.correct(request)).rejects.toThrow('interrupt after receipt')

    await harness.memories.update('mem_transient', { disabled: true }, { workspace: 'workspace-a' })
    await expect(harness.service.ready()).resolves.toBeUndefined()
    await expect(harness.service.ready()).resolves.toBeUndefined()

    await harness.memories.update('mem_transient', { disabled: false }, { workspace: 'workspace-a' })
    const result = await harness.service.correct(request)
    expect(result.replayed).toBe(false)
    expect(await harness.feedback.aggregate('mem_transient')).toMatchObject({ correctionCount: 1 })
  })

  it('quarantines an unreadable correction receipt instead of failing every startup', async () => {
    const harness = await createHarness()
    const receiptDir = join(harness.root, 'memory-feedback-corrections')
    await mkdir(receiptDir, { recursive: true })
    await writeFile(join(receiptDir, `correction-${'a'.repeat(32)}.json`), '{not json', 'utf8')

    await expect(harness.service.ready()).resolves.toBeUndefined()
    await expect(harness.service.ready()).resolves.toBeUndefined()
    expect(await readdir(receiptDir)).toEqual([`correction-${'a'.repeat(32)}.json.corrupt`])
  })

  it('prunes terminal correction receipts to a bounded tail', async () => {
    const harness = await createHarness()
    const receiptDir = join(harness.root, 'memory-feedback-corrections')
    await mkdir(receiptDir, { recursive: true })
    for (let index = 0; index < 70; index += 1) {
      await writeFile(join(receiptDir, `correction-${index.toString(16).padStart(32, '0')}.json`), JSON.stringify({
        schemaVersion: 1,
        operationId: `op_${index}`,
        requestHash: 'a'.repeat(64),
        previousMemoryId: `mem_${index}`,
        replacementMemoryId: `mem_new_${index}`,
        correctedAt: new Date(Date.parse(NOW) + index * 1000).toISOString(),
    ...
    expect(entries.some((name) => name === `correction-${'0'.repeat(32)}.json`)).toBe(false)
  })

  it('uses a bounded typed service error', () => {
    expect(new MemoryFeedbackServiceError('unavailable', 'denied')).toMatchObject({
      name: 'MemoryFeedbackServiceError', code: 'unavailable', message: 'denied'
    })
  })
})
