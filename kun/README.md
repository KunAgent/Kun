...
- The GUI marketplace includes a Context.dev preset at `https://mcp.context.dev/mcp` using streamable HTTP and OAuth 2.1 dynamic client registration with PKCE. Leaving `clientId` and `clientSecret` unset is intentional; the encrypted MCP OAuth store keeps tokens out of `mcp.json`. The preset describes the public search, news, scraping, extraction, document, brand, screenshot, monitor, and batch capabilities and does not configure the unreleased Answers API. Treat returned web content as untrusted before using it in prompts or side effects.
