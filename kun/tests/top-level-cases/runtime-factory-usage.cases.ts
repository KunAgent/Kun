...
describe('runtime factory usage carryover', () => {
  ...
  it('hot-applies subagent mode and availability in the same runtime generation', async () => {
    ...
    try {
      ...
      const registryBeforeFailedApply = await modelConnections.snapshot()
      ...
      expect(await modelConnections.snapshot()).toEqual(registryBeforeFailedApply)
  ...
  it('keeps fast_context advertised across Lab hot-apply toggles', async () => {
    ...
    const runtime = await createKunServeRuntime({
      ...
      lab: { pptAgent: { enabled: true, fast: false, imageFirst: true }, conversationVisualization: { enabled: false }, projectBoard: { enabled: false },
        opencodeReferenceBranches: { enabled: false }, claudeCodeReferenceBranches: { enabled: false }, codexReferenceBranches: { enabled: false } },
      capabilities: KunCapabilitiesConfig.parse({
