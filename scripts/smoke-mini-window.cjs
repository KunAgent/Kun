...
async function main() {
  ...
  try {
    ...
    assert.equal(await page.locator('[data-workbench-left-sidebar]').isVisible(), false)
    assert.equal(await page.locator('[data-workbench-right-panel]').isVisible(), false)
    ...
    assert.equal(await page.locator('[data-workbench-left-sidebar]').isVisible(), true)
